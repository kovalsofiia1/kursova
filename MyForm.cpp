...
System::Void kursova::MyForm::button1_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	try {
		...
		int pos = x_inp.find(",");
		if (pos != -1 && x_inp.find(",", pos+1) != -1) {
			x_input->Text = "";
			throw "Decimal point in x entered more than once!";
		}
		pos = y_inp.find(",");
		if (pos != -1 && y_inp.find(",", pos + 1) != -1) {
			y_input->Text = "";
			throw "Decimal point in y entered more than once!";
		}
		pos = x_inp.find("-");
		if (pos != -1 && x_inp.find("-", pos + 1) != -1) {
			x_input->Text = "";
			throw "Minus in x entered more than once!";
		}
		pos = y_inp.find("-");
		if (pos != -1 && y_inp.find("-", pos + 1) != -1) {
			y_input->Text = "";
			throw "Minus in y entered more than once!";
		}

		if (x_inp.find("-") != -1 && x_inp.find("-") > 0 ) {
			
			x_input->Text = "";
			
			throw "Minus in x in the wrong place!";
		}
		if ( y_inp.find("-") != -1 && y_inp.find("-") > 0) {

			y_input->Text = "";
			throw "Minus in y in the wrong place!";
		}
		if (x_inp.find(",") != -1 && x_inp.find(",") == 0 ) {
			
			x_input->Text = "";
			
			throw "Decimal point in x in the wrong place!";
		}
		if ( y_inp.find(",") != -1 && y_inp.find(",") == 0) {
			y_input->Text = "";
			throw "Decimal point in y in the wrong place!";
		}
		if (p->GetSize() > 20) {
			throw "Maximum 20 points!";
		}
		if (Convert::ToDouble(x1) > 1000 || Convert::ToDouble(x1) < -1000) {
			throw "Maximum x value is 1000 and minimum is -1000!";
		}
		if (Convert::ToDouble(y1) > 1000 || Convert::ToDouble(y1) < -1000) {
			throw "Maximum y value is 1000 and minimum is -1000!";
		}
		
			xx = Convert::ToDouble(x1);
			yy = Convert::ToDouble(y1);
			p->check_if_present(xx);
			if (p->check_if_present(xx) == true) {
				throw "You can't have two points with the same x coordinate!";
			}
...
System::Void kursova::MyForm::button2_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		if (p->GetSize() < 1) {
			MessageBox::Show("You have not entered any points. You need minimum 2");
		}
		if (p->GetSize() == 1) {
...
void kursova::MyForm::build_graphic(CubicSplineInterpolation* inter) {
			   ...
			   double a = p->GetX(0), b = p->GetX(p->GetSize() - 1), h = 0.01, x, y;
			   if (b - a > 100) {
				   h = 0.1;
			   }
			   chart->Series[0]->Points->Clear();
